...
public class Minecraft {
	...
	private static ClassChecker[] classChecks = new ClassChecker[] {
			...
			(new ClassChecker() {
				...
				public void check(Minecraft m, ByteClass bClass) {
					if (bClass.fields.length != 3)
						return;
					int privateStatic = AccessFlags.PRIVATE
							| AccessFlags.STATIC;
					for (int i = 0; i < 3; i++) {
					...
					if ((bClass.constructorCount == 0)
							&& (bClass.methodCount == 6)
			...
			new CCRequire(new CCPropertyPreset("WorldType", "a", "types", "b",
					"default", "c", "flat", "d", "largeBiomes", "e",
					"amplified", "g", "default_1_1", "f", "customized"),
					"WorldType"),
			new CCRequire(new CCMethodPreset("BlockInit", "c()", "initialize"),
					"BlockInit"),
			new CCRequire(new CCMethodPreset("GenLayer", "a(long, @WorldType)",
					"initializeAllBiomeGenerators",
					"a(long, @WorldType, String)",
					"initializeAllBiomeGeneratorsWithParams",
					"a(int, int, int, int)", "getInts"), "GenLayer"),
			new CCRequire(new CCMulti(new CCMethodPreset("IntCache", "a(int)",
					"getIntCache", "a()", "resetIntCache", "b()",
					"getInformation"), new CCPropertyPreset("IntCache", "a",
					"intCacheSize", "b", "freeSmallArrays", "c",
					"inUseSmallArrays", "d", "freeLargeArrays", "e",
					"inUseLargeArrays")), "IntCache") };
			...
			while (enu.hasMoreElements()) {
				...
				if (!entry.isDirectory() && (nameSplit.length == 2)
					...
					if (is.available() < 8000) { // TODO: Double check that this
						...
						is.read(classData);
						is.close();
						byteClassStack.push(new ByteClass(nameSplit[0],
								classData));
					}
				}
			}
	...
	/*
	 * This was the old search-and-add-all libraries method. This may still be
	 * useful if the user doesn't have a json file, or mojang changes the
	 * format.
	 * 
	 * private Stack<URL> getLibraries(File path, Stack<URL> urls) { File[]
	 * files = path.listFiles(); for (int i = 0; i < files.length; i++) { if
	 * (files[i].isDirectory()) { getLibraries(files[i], urls); } else { try {
	 * Log.i("Found library: " + files[i]); urls.push(files[i].toURI().toURL());
