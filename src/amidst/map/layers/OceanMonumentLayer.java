...
public class OceanMonumentLayer extends IconLayer {
	...
	public static List<Biome> validBiomes = Arrays.asList(new Biome[] {
			Biome.deepOcean, Biome.deepOceanM, // Not sure if the extended
												// biomes count
	});
	public static List<Biome> validSurroundingBiomes = Arrays
			.asList(new Biome[] { Biome.ocean, Biome.deepOcean,
					Biome.frozenOcean, Biome.river, Biome.frozenRiver,
					// Not sure if the extended biomes count
					Biome.oceanM, Biome.deepOceanM, Biome.frozenOceanM,
					Biome.riverM, Biome.frozenRiverM, });
		...
		for (int x = 0; x < size; x++) {
			for (int y = 0; y < size; y++) {
				...
				if (checkChunk(chunkX, chunkY)) {
					frag.addObject(new MapObjectOceanMonument(x << 4, y << 4)
							.setParent(this));
				}
			}
		}
	...
	public void setRandomSeed(int a, int b, int structure_magic_number) {
		long positionSeed = (long) a * 341873128712L + (long) b * 132897987541L
				+ Options.instance.seed + (long) structure_magic_number;
	...
	public boolean checkChunk(int chunkX, int chunkY) {
		...
		int structureMagicNumber = 10387313; // 10387313 is the magic salt for
												// ocean monuments

		int chunkXadj = chunkX;
		int chunkYadj = chunkY;
		if (chunkXadj < 0)
			chunkXadj -= maxDistanceBetweenScatteredFeatures - 1;
		if (chunkYadj < 0)
			chunkYadj -= maxDistanceBetweenScatteredFeatures - 1;
		...
		int distanceRange = maxDistanceBetweenScatteredFeatures
		...
		if ((chunkX == a) && (chunkY == b)) {
			...
			result = MinecraftUtil.getBiomeAt(chunkX * 16 + 8, chunkY * 16 + 8) == Biome.deepOcean
					&& MinecraftUtil.isValidBiome(chunkX * 16 + 8,
							chunkY * 16 + 8, structureSize,
							validSurroundingBiomes);
		}
		return result;
	}
}
