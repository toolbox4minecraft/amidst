...
/** 
 * A BufferedImage which provides OutOfFrameImage information, based
 * off a crude analysis of the image. See constructor comment.
 */
public class VerticalOutOfFrameImage extends BufferedImage implements
	...
	public VerticalOutOfFrameImage(BufferedImage image, int frameOffsetX, int frameOffsetY, int frameWidth, int frameHeight) {
		super(
			image.getColorModel(),
			image.copyData(null),
	...
	/**
	 * Use pixel transparency along the left edge of the icon to 
	 * automatically determine the vertical frame position. 
	 * Assumes the image has no horizontal out of frame effects.
	 * 
	 * If you want transparency on the left side of the image 
	 * without adjusting its frame, or if you want to control 
	 * where the frame starts, use a pixel that is mostly transparent.
	 * (Spawn.png and Witch.png do this)
	 */
	public VerticalOutOfFrameImage(BufferedImage image) {
		...
		this.frameHeight = Math.max(0, getHeight() - (imageMarginTop + imageMarginBottom));	
	}
