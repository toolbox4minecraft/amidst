...
public abstract class TextWidget extends Widget {
	...
	protected void doUpdate(FontMetrics fontMetrics, float time) {
		...
		if (newTextLines != null && !newTextLines.equals(textLines)) {
			...
			for (String line : textLines) {
				...
				if (line != null) {
					maxStringWidth = Math.max(maxStringWidth, fontMetrics.stringWidth(line));
				}
			}
			
			setWidth(getMarginLeft() + maxStringWidth + getMarginRight());
			setHeight(
				Math.max(getMinimumHeight(), getMarginTop() + stringHeight + getMarginBottom())
			);
		}
	...
	protected void doDraw(Graphics2D g2d) {
	    ...
	    for (String line : textLines) {		
			if (line != null) g2d.drawString(line, x, y);
	...
	protected String updateText() {
		...
		throw new RuntimeException("One of either updateMultilineText() or updateText() must be overridden");		
	}
}
